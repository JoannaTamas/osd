    ...
    )
{
    ...
    if (!SUCCEEDED(status))
    {
        LOG_FUNC_ERROR("_ThreadInit", status);
        return status;
	}else  GetCurrentThread()->CHILD_THREADS++;
...
#pragma warning(suppress:4152)
    ...
    *Thread = pThread;
	...
	//ex1
	pThread->CPU_IDENTIFIER = pCpu->ApicId;
	LOG("Name: &s, TID: %x\n", pThread->Name, pThread->CPU_IDENTIFIER);

	//ex2.1

	CRT_TH_ID = GetCurrentThread()->Id;
	CRT_CPU_ID = GetCurrentPcpu()->ApicId;
	NEW_TH_ID = pThread->Id;


	LOG("Thread: [ NEW_TH_ID= %x] is the Xth: %d thread created by thread[CRT_TH_ID= %x]  on CPU[CRT_CPU_ID= %x]\n",
		NEW_TH_ID, GetCurrentThread()->CHILD_THREADS, CRT_TH_ID, CRT_CPU_ID);
    ...
    )
{
    ...
    if (++pCpu->ThreadData.RunningThreadTicks >= pThread->TICK)
    {
		//3.2
		pThread->TIME_SLICE++;
    ...
    )
{
	...
	CRT_TH_ID = pThread->Id;
	PR_TH_ID = GetCurrentThread()->Id;
	CRT_CPU_ID = GetCurrentPcpu()->ApicId;
	CPU_ID = pThread->CPU_IDENTIFIER;

	pThread->CHILD_THREADS--; 



	LOG("Thread[CRT_TH_ID= %x] created on CPU[CPU_ID= %x] is finishing on CPU[CRT_CPU_ID= %x], while its parent thread[PR_TH_ID= %x] still has more X=%d child threads\n",
		CRT_TH_ID,CPU_ID,CRT_CPU_ID,PR_TH_ID, pThread->CHILD_THREADS);

	//ex 3.3
	LOG("Thread [CRT_TH_ID= %x] was allocated X = %u time quanta of length Y= %u \n", CRT_TH_ID, pThread->TIME_SLICE, pThread->TICK * pThread->TIME_SLICE);
